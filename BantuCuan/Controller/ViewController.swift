...
class ViewController: UIViewController, UITableViewDelegate{
    ...
    @IBAction func settingPressed(_ sender: UIButton) {
        ...
        let action = UIAlertAction(title: "Suara A", style: .default) { (action) in
            self.setSound(musicName: "sound1")
            self.dismiss(animated: true, completion: nil)
        }
        let action2 = UIAlertAction(title: "Suara B", style: .default) { (action) in
            self.setSound(musicName: "sound2")
            self.dismiss(animated: true, completion: nil)
        }
        let action3 = UIAlertAction(title: "Suara C", style: .default) { (action) in
            self.setSound(musicName: "sound3")
            self.dismiss(animated: true, completion: nil)
        }
        let action4 = UIAlertAction(title: "Kembali", style: .destructive) { (action) in
            self.dismiss(animated: true, completion: nil)
        }
    ...
    @IBAction func addPressed(_ sender: UIButton) {
        ...
        let alert = UIAlertController(title: "Tambahkan \(currentCryptoOfSelectedPicker) ke Crypto Watchlist!", message: "Masukan nilai batas bawah dan batas atas nilai crypto yang kalian inginkan dalam mata uang rupiah. Contoh: 2314.15123", preferredStyle: .alert)
        
        let action = UIAlertAction(title: "Tambahkan", style: .default) { [self] (action) in
            if let atasText = textFieldBatasAtas.text, let bawahText = textFieldBatasBawah.text {
                if let atasTextDouble = Double(atasText), let bawahTextDouble = Double(bawahText){
                    ...
                    let alert1 = UIAlertController(title: "Eror!", message: "Format nilai batas atas dan batas bawah harus dalam format angka dan tidak boleh kosong ya. Contoh: 12435.86349", preferredStyle: .alert)
        ...
        let action1 = UIAlertAction(title: "Kembali", style: .destructive) { (action) in
